...
func (t *PostgresTaskRepository) CreateTask(ctx context.Context, task *models.TaskCreate) (int64, error) {
	...
	var lastID int64
	err := t.db.QueryRowContext(ctx, addTaskQuery, task.Title, task.Description, task.Price).Scan(&lastID)
	if err != nil {
...
func (r *PostgresTaskRepository) CompleteTask(ctx context.Context, userId, taskId int64) error {
	...
	if _, err = tx.ExecContext(ctx, completeQuery, userId, taskId); err != nil {
		return errors.NewInternal("failed to complete task", err)
	}
	// Обновляем баланс пользователя
	if _, err = tx.ExecContext(ctx, balanceToUserQuery, task.Price, userId); err != nil {
...
func (r *PostgresTaskRepository) GetAllTasks() ([]models.Task, error) {
	...
	rows, err := r.db.QueryContext(context.Background(), "SELECT task_id, title, description FROM tasks")
	if err != nil {
		return nil, errors.NewInternal("Error executing query", err)
	}
	defer rows.Close()
